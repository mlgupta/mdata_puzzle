...
def node_weight(index):
    ...
    next_idx = index + 3
    if (next_idx < input_length):
        left_child_node = node_weight(next_idx)
    else:
        return node

    next_idx = index + 4
    if (next_idx < input_length):
        right_child_node = node_weight(next_idx)
    else:
        return node
    
    left_total_weight = input[index] + left_child_node["weight"]
    right_total_weight = input[index] + right_child_node["weight"]

    if (input[index] < left_total_weight):
        if (left_total_weight < right_total_weight):
            node["weight"] = right_total_weight
            node["path"] += right_child_node["path"]
        else:
            node["weight"] = left_total_weight
            node["path"] += left_child_node["path"]
    else:
        if (input[index] < right_total_weight):
            node["weight"] = right_total_weight
            node["path"] += right_child_node["path"]
...
tree_weight = node_weight(0)
print(tree_weight["weight"])
print(tree_weight["path"])
